...
carro.adicionarPessoas = function ( n ) {
  ...
  if (carro.assentos === carro.quantidadePessoas && total >= carro.assentos) {
    return 'O carro já está lotado!';
  }

  if (total > carro.assentos){
    var vagasRestantes = carro.assentos - carro.quantidadePessoas;
    var pluralPessoa = vagasRestantes == 1 ? 'pessoa' : 'pessoas';
    var pluralCabe = vagasRestantes == 1 ? 'cabe' : 'cabem';

    return 'Só ' + pluralCabe + ' mais ' + vagasRestantes + ' ' + pluralPessoa + '!';
  }
